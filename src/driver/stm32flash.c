...
void HAL_SPI_TransmitReceive(struct spi_device *SpiHandle, uint8_t *send, uint8_t *receive, int length, int timeout)
{
  spi_xfer.tx_buf = send;
  spi_xfer.rx_buf = receive,
  spi_xfer.len = length;
  spi_xfer.speed_hz = 4000000;

  spi_message_init(&spi_message);
  spi_message_add_tail(&spi_xfer, &spi_message);
  spi_sync(SpiHandle, &spi_message);
}

void HAL_SPI_Transmit(struct spi_device *SpiHandle, uint8_t *send, int length, int timeout)
{
  spi_xfer.tx_buf = send;
...
void HAL_SPI_Receive(struct spi_device *SpiHandle, uint8_t *receive, int length, int timeout)
{
...
/**
  ...
  * @retval None
  */
int BL_Init(struct spi_device *hspi)
{
